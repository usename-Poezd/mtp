...
Функция `apply_multiple_functions()` применяет произвольное количество функций к одному списку данных. Реализована через циклы for и метод append.
...
**Реализация:** Использует вложенные циклы для применения каждой функции к каждому элементу списка.

### 2. Фильтрация чётных чисел из списка

Функция `filter_even_numbers()` отбирает только чётные числа из списка. Реализована через цикл for и проверку остатка от деления.
...
**Реализация:** Проходит по списку и добавляет в результат только те элементы, у которых `x % 2 == 0`.

### 3. Декоратор замера времени работы функции

Декоратор `@timing_decorator` измеряет время выполнения функции и выводит информацию о времени выполнения.
...
# Выводит: Function slow_function took 0.100123 seconds
# result = {
#     'result': 'Done',
...
**Реализация:** Использует `time.time()` для измерения времени до и после выполнения функции.
...
- Корректно обрабатывает пустые последовательности (выбрасывает ошибку, если нет initializer)
- Использует цикл while для последовательного применения функции
- Работает с любыми типами данных

**Реализация:** Проверяет длину списка, устанавливает начальное значение аккумулятора, затем применяет функцию в цикле while.
...
**Реализация:** Проверяет количество переданных аргументов через `func.__code__.co_argcount`. Если аргументов недостаточно, возвращает функцию для частичного применения.
...
```python
from functional_programming import compose, square, double

composed = compose(square, double)
result = composed(3)  # double(3) = 6, square(6) = 36
```

**Реализация:** Применяет функции в обратном порядке через цикл for с обратной итерацией.
...
**Реализация:** Использует цикл while для генерации квадратов чисел от 0 до n.
...
**Реализация:** Использует цикл while и временную переменную для вычисления следующего числа Фибоначчи.
...
В данной работе функции реализованы через обычные циклы для лучшего понимания принципов работы.
...
Код написан в учебном стиле с использованием простых конструкций:
- Циклы for и while вместо сложных функциональных конструкций
- Понятные имена переменных
- Комментарии на русском языке
- Простая структура без излишней абстракции

Это помогает лучше понять принципы работы функционального программирования на практике.
